...
static int
match_ipv6_addresses(char *reverse_ip, char *intf_ip)
{
	int i = 0, j = 0, idx = 0;
	...
	if (inet_ntop(AF_INET6, buf, temp_ip, INET6_ADDRSTRLEN) == NULL)
		return 0;


	if (strcmp(temp_ip, intf_ip))
		return 0;
		...
		uint8_t *orig_buffer, int orig_len)
{
	...
	for (ifa = ifap; ifa; ifa = ifa->ifa_next) {
		...
		if (ifa->ifa_addr->sa_family == AF_INET6) {
			...
			if (match_ipv6_addresses(reverse_ip, intf_ip)) {
				len = dn_comp(mdns_hostname_local, buffer, sizeof(buffer), NULL, NULL);

				if (len < 1)
					continue;

				dns_add_answer(TYPE_PTR, buffer, len, ttl);
			}
		}
	}
	dns_send_answer(iface, to, name);
